...
export default function AgentAppointmentsPage() {
  ...
  const fetchAppointments = async () => {
    setLoading(true);
    const { error, data } = await apiClient(`appointments?agent=${session.user.id}&page=0&limit=100`, { session });
    ...
    <div className="max-w-7xl mx-auto pb-12">
      ...
      {loading ? (
        ...
        <div className="bg-surface-container-lowest p-12 text-center hairline-all">
          <p className="font-body-lg text-on-surface-variant mb-6">You don't have any appointments scheduled yet.</p>
        </div>
      ) : (
